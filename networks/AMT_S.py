...
class Model(nn.Cell):
    def __init__(self, 
        ...
        self.comb_block = nn.SequentialCell(
            nn.Conv2d(3*num_flows, 6*num_flows, 3, stride=1, padding=1, pad_mode='pad', has_bias=True),
            nn.PReLU(6*num_flows),
            nn.Conv2d(6*num_flows, 3, 3, stride=1, padding=1, pad_mode='pad', has_bias=True)
        )
