...
def bilinear_sampler(img, coords, mask=False):
    ...
    grid = ops.cat([xgrid, ygrid], axis=-1)
    img = ops.grid_sample(img, grid, align_corners=True)
...
class SmallUpdateBlock(nn.Cell):
    def __init__(self, cdim, hidden_dim, flow_dim, corr_dim, fc_dim,
        ...
        #     nn.Conv2d(fc_dim+4+cdim, hidden_dim, 3, padding=1, pad_mode='pad'),
        #     nn.LeakyReLU(negative_slope=0.1, inplace=True),
        #     nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, pad_mode='pad'),
        # )
        ...
        self.convc1 = nn.Conv2d(2 * cor_planes, corr_dim, 1, padding=0, pad_mode='pad', has_bias=True)
        self.convf1 = nn.Conv2d(4, flow_dim*2, 7, padding=3, pad_mode='pad', has_bias=True)
        self.convf2 = nn.Conv2d(flow_dim*2, flow_dim, 3, padding=1, pad_mode='pad', has_bias=True)
        self.conv = nn.Conv2d(corr_dim+flow_dim, fc_dim, 3, padding=1, pad_mode='pad', has_bias=True)

        self.gru = nn.SequentialCell(
            nn.Conv2d(fc_dim+4+cdim, hidden_dim, 3, padding=1, pad_mode='pad', has_bias=True),
            nn.LeakyReLU(alpha=0.1),
            nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, pad_mode='pad', has_bias=True),
        )

        self.feat_head = nn.SequentialCell(
            nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, pad_mode='pad', has_bias=True),
            nn.LeakyReLU(alpha=0.1),
            nn.Conv2d(hidden_dim, cdim, 3, padding=1, pad_mode='pad', has_bias=True),
        )

        self.flow_head = nn.SequentialCell(
            nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, pad_mode='pad', has_bias=True),
            nn.LeakyReLU(alpha=0.1),
            nn.Conv2d(hidden_dim, 4, 3, padding=1, pad_mode='pad', has_bias=True),
        )

        self.lrelu = nn.LeakyReLU(alpha=0.1)
            
    # def forward(self, net, flow, corr):
    def construct(self, net, flow, corr):
        ...
        inp = ops.cat([inp, flow, net], axis=1)
...
class BasicUpdateBlock(nn.Cell):
    def __init__(self, cdim, hidden_dim, flow_dim, corr_dim, corr_dim2, 
        ...
        self.convc1 = nn.Conv2d(2 * cor_planes, corr_dim, 1, padding=0, pad_mode='pad', has_bias=True)
        self.convc2 = nn.Conv2d(corr_dim, corr_dim2, 3, padding=1, pad_mode='pad', has_bias=True)
        self.convf1 = nn.Conv2d(4, flow_dim*2, 7, padding=3, pad_mode='pad', has_bias=True)
        self.convf2 = nn.Conv2d(flow_dim*2, flow_dim, 3, padding=1, pad_mode='pad', has_bias=True)
        self.conv = nn.Conv2d(flow_dim+corr_dim2, fc_dim, 3, padding=1, pad_mode='pad', has_bias=True)
        ...
        self.flow_head = nn.SequentialCell(
            nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, pad_mode='pad', has_bias=True),
            nn.LeakyReLU(alpha=0.1),
            nn.Conv2d(hidden_dim, 4*out_num, 3, padding=1, pad_mode='pad', has_bias=True),
        )
...
class BidirCorrBlock:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4):
        ...
        for _ in range(self.num_levels-1):
            ...
            corr = ops.avg_pool2d(corr, 2, stride=2)
            corr_T = ops.avg_pool2d(corr_T, 2, stride=2)
            self.corr_pyramid.append(corr)
            self.corr_pyramid_T.append(corr_T)
    ...
    def __call__(self, coords0, coords1):
        ...
        for i in range(self.num_levels):
            ...
            start = Tensor(-r, ms.float32)
            limit = Tensor(r, ms.float32)
            delta = Tensor(2*r+1, ms.int32)
            dx = ops.linspace(start, limit, delta)
            dy = ops.linspace(start, limit, delta)
            delta = ops.stack(ops.meshgrid(dy, dx, indexing='ij'), axis=-1)
        ...
        out = ops.cat(out_pyramid, axis=-1)
        out_T = ops.cat(out_pyramid_T, axis=-1)    
