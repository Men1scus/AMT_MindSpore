...
def convrelu(in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True):
    ...
    return nn.SequentialCell(
        nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding=padding, dilation=dilation, group=groups, has_bias=bias, pad_mode='pad'), 
        nn.PReLU(out_channels)
    )
...
class ResBlock(nn.Cell):
    def __init__(self, in_channels, side_channels, bias=True):
        ...
        self.conv1 = nn.SequentialCell(
            nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1, has_bias=bias, pad_mode='pad'), 
            nn.PReLU(in_channels)
        )
        self.conv2 = nn.SequentialCell(
            nn.Conv2d(side_channels, side_channels, kernel_size=3, stride=1, padding=1, has_bias=bias,  pad_mode='pad'), 
            nn.PReLU(side_channels)
        )
        ...
        self.conv5 = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1, pad_mode='pad', has_bias=bias)
        self.prelu = nn.PReLU(in_channels)
    ...
    def construct(self, x):
        ...
        out = self.conv3(ops.cat([res_feat, side_feat], 1))

        res_feat = out[:, :-self.side_channels, ...]
        side_feat = out[:, -self.side_channels:, :, :]
        side_feat = self.conv4(side_feat)
        # out = self.conv5(torch.cat([res_feat, side_feat], 1))
        out = self.conv5(ops.cat([res_feat, side_feat], 1))
...
class Encoder(nn.Cell):
    ...
    def construct(self, in_x):
        ...
        for idx in range(len(self.channels)):
            ...
            fs.append(out_x)
            in_x = out_x
        # for layer in self.pyramid:  # 直接遍历pyramid中的层
        #     in_x = layer(in_x)
        #     fs.append(in_x)
        return fs
    
# class InitDecoder(nn.Module):
class InitDecoder(nn.Cell):
    def __init__(self, in_ch, out_ch, skip_ch) -> None:
        ...
        self.convblock = nn.SequentialCell(
            ...
            nn.Conv2dTranspose(in_ch*2, out_ch+4, kernel_size=4, stride=2, padding=1, pad_mode='pad', has_bias=True)
    ...
    def construct(self, f0, f1, embt):
        ...
        # flow0, flow1 = tops.chunk(out[:, :4, ...], 2, 1)
        flow0, flow1 = ops.chunk(out[:, :4, ...], 2, 1)

        ft_ = out[:, 4:, ...]
        return flow0, flow1, ft_
    
# class IntermediateDecoder(nn.Module):
class IntermediateDecoder(nn.Cell):
    def __init__(self, in_ch, out_ch, skip_ch) -> None:
        super().__init__()
        ...
        self.convblock = nn.SequentialCell(
            ...
            ResBlock(in_ch*3, skip_ch), 
            # nn.ConvTranspose2d(in_ch*3, out_ch+4, 4, 2, 1, bias=True)
            nn.Conv2dTranspose(in_ch*3, out_ch+4, kernel_size=4, stride=2, padding=1, pad_mode='pad', has_bias=True)
        )
    # def forward(self, ft_, f0, f1, flow0_in, flow1_in):
    def construct(self, ft_, f0, f1, flow0_in, flow1_in):
        f0_warp = warp(f0, flow0_in)
        f1_warp = warp(f1, flow1_in)
        # f_in = torch.cat([ft_, f0_warp, f1_warp, flow0_in, flow1_in], 1)
        f_in = ops.cat([ft_, f0_warp, f1_warp, flow0_in, flow1_in], 1)
